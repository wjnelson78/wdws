# SPRINT A — Critical Privilege and Privacy Gates (v2.2)

**Work Order ID:** `SPRINT_A_PRIVILEGE_PHI`
**Reference:** Athena Strategic Framework v2.0, Section 9.1
**Supersedes:** SPRINT_A_PRIVILEGE_PHI (v1, v2, v2.1 drafts — all retained for audit trail)
...
### v2.2 → v2.1 (this revision)

- **§9 rollback step 5 fixed.** Only rows where `approved IS NULL` get flipped to `approved=false, rejection_note='sprint_aborted'`. Already-approved rows are left intact — the rollback narrative in the step-7 finding records that downstream work was reverted, without falsifying the approval audit trail.
- **§10 rule 9 added.** Conversational-approval mechanics: when Will approves in live conversation, the agent immediately records the approval to `ops.pending_approvals` with `approved=true, approved_by='will', approval_method='conversation'`. The polling convention in rule 1 is otherwise unchanged — the table stays the single source of truth for approval state.
- **New preflight migration `026_ops_approval_method.sql`** (§2.1) adds the `approval_method text` column to `ops.pending_approvals` so rule 9's mechanics are expressible. This is a preflight step, not a Task, because it's substrate for Sprint A's own operation.
- **Migration numbers shifted.** Legal privilege: 026 → 027. Medical PHI: 027 → 028. Preflight substrate occupies 026. Task numbers (Task 1–5) are unchanged.

### v2.1 → v2

- **Test 18 fixed.** Morning-brief email reads apply metadata-only defaults to PHI-in-email-body cases (IME scheduling, records requests, clinical communications from opposing experts), not full exclusion. Matches the general morning-brief rule at §5.5.
- **Pre-auto-classify pattern sanity check added** at §6.2. Agent surfaces 10 random matches to Will in an informal `category='sprint_a_keyword_pattern_review'` finding before the mechanical classification pass runs on production data. Not a formal stop-gate.
- **Task 5 stop-gate clarified** at §7.3. T5_REVIEW can be satisfied by manual one-shot invocation OR scheduled run — decoupled from `wdws-agents.service` health to avoid unnecessary blocking.
- **100-docs/hour ceiling realism note** added at §6.3. 100/hr is a ceiling not a sustained target; interactive-load contention makes ~7 days the realistic completion estimate for Task 4, still inside the 14-day window.
- **Quiet-hours rule clarified** at §10 rule 6. "Autonomous" explicitly defined as agent-initiated without current user direction; live-conversation work is always directed.
- **Canonical-set design note added** to §12.3 closeout recommendations. The eventual MCP/dashboard refactor is likely "both import from a shared retrieval library + own presentation layer" rather than "dashboard imports from MCP."
...
- [ ] Preflight migration `026_ops_approval_method.sql` applied (see §2.1). After this step, `\d ops.pending_approvals` shows `approval_method text` column.
...
### 2.1 Preflight Migration — `026_ops_approval_method.sql`

Sprint A's conversational-approval mechanics (§10 rule 9) require a new column on `ops.pending_approvals`. Because the column must exist before T1's stop-gate machinery can function, this is preflight substrate, not a Task.

Agent writes `/opt/wdws/migrations/026_ops_approval_method.sql`:

```sql
-- Migration 026: Add approval_method column to ops.pending_approvals
-- Sprint A preflight substrate
-- Additive only.
...
COMMENT ON COLUMN ops.pending_approvals.approval_method IS
    'How approval was recorded. Values: conversation (directed chat), dashboard (UI click), api (direct row update), bootstrap (pre-rule-9 migrations). NULL on rows created before this column existed.';

COMMIT;
```

And `/opt/wdws/migrations/026_ops_approval_method_down.sql`:
...
**Chicken-and-egg resolution.** This migration itself cannot go through the conversational-approval flow (the column it adds is what that flow depends on). It is applied during preflight using the migration runner with `applied_by='sprint_a_bootstrap'` and, inside the runner's INSERT into `ops.migrations_applied`, does not touch `ops.pending_approvals` at all — no gate is created for it. Will authorizes this single exception in conversation; the bootstrap status is recorded in the migration's `analysis` field.

After this migration lands, all subsequent Sprint A approvals (T1–T5 and the closeout) go through rule 9's conversational-approval flow with `approval_method='conversation'`.

### 2.2 Preflight existing-row backfill (trivial)

Pre-existing rows on `ops.pending_approvals` (one row observed in preflight) keep `approval_method = NULL`. No backfill required — NULL legitimately means "created before the column existed." Reports and dashboards that filter on `approval_method` must treat NULL as a fourth case (historical) distinct from the three defined values.

---

## 3. Task 1 — Draft Migration 027 (Legal Privilege + Judge Fields)
...
Agent writes to `/opt/wdws/migrations/027_legal_privilege_judge.sql`:
...
Agent writes to `/opt/wdws/migrations/027_legal_privilege_judge_down.sql`:
...
## 4. Task 2 — Draft Migration 028 (Medical PHI Classification)
...
Agent writes to `/opt/wdws/migrations/028_medical_phi_classification.sql`:
...
Agent writes to `/opt/wdws/migrations/028_medical_phi_classification_down.sql`:
...
18. `morning_briefing.get_recent_important_emails` applies metadata-only defaults to any pulled email whose body references PHI (IME scheduling, records requests, expert-to-counsel clinical communications). Surfaces subject line, sender, date, case linkage; never pulls email body into the brief narrative, never pulls attachment content. Matches the general morning-brief rule at §5.5 — PHI in a legal-domain email is still PHI.
...
**Pattern sanity check (pre-execution).** Before the mechanical pass runs on production data, agent executes the pattern set in read-only mode against the corpus and writes a finding with `severity='info'`, `category='sprint_a_keyword_pattern_review'`, and an `evidence` JSON containing 10 randomly sampled match rows (`document_id`, `filename`, `title`, matched pattern, 200-char context window around the match). Will confirms that the matches are correctly privileged in an informal review (target: under 5 minutes of reading). The third pattern in particular — `'PRIVILEGED AND CONFIDENTIAL'` + proximity to `'attorney'` / `'counsel'` — is the one most likely to over-trigger on non-privileged business correspondence; Will should scrutinize it closely. On confirmation, the mechanical pass runs. On rejection, the pattern set is revised and the sanity check repeats. This is a sanity check, not a formal stop-gate — no `ops.pending_approvals` row is created.
...
4. **Throughput realism.** 100 docs/hour is a **ceiling**, not a sustained target. Actual throughput depends on interactive-load contention on the model router — the backfill worker shares capacity with live chat, and during Will's drafting hours the effective rate is expected to drop to roughly 40–60 docs/hour. Include T4 stop-gate pause time for the sample review as well. Realistic end-to-end completion for the 10,344 in-scope documents is **~7 days wall-clock**, not the theoretical 4.3. Still comfortably inside the 14-day sprint, but the agent should not optimize Task 4 pacing based on the 4.3-day figure or surface false "ahead of schedule" signals. If throughput falls below ~30 docs/hour for two consecutive hours during business hours, that is expected contention, not a fault condition.
...
**Stop-gate: T5_REVIEW**
T5_REVIEW can be satisfied by either path:

- **(a) Manual one-shot invocation.** Human (or agent under direct conversational direction) triggers `generate_disclosures_report`, `generate_privilege_summaries`, and `check_expired_authorizations` via CLI, notebook, or synchronous tool call. Reports land in `/opt/wdws/reports/weekly/YYYY-MM-DD/`. Will reviews via an `ops.pending_approvals` entry with `decision_type='sprint_a_t5_review'`. This path lets Task 5 complete synchronously if the `wdws-agents.service` fix slips or is explicitly deferred.
- **(b) Scheduled run** after `wdws-agents.service` is healthy. The agent's Sunday 3 AM cron produces the reports; Will reviews them through the same `ops.pending_approvals` entry. This is the normal path and the one that Sprint A converges on long-term.

Either path satisfies the gate. The gate is **not** blocked on service health — only the activation of the recurring schedule is. On approval via either path, the recurring Sunday 3 AM schedule is activated once (and only once) `wdws-agents.service` is healthy; until then the reports are generated on-demand per path (a).

**Per §10 rule 7,** the *recurring* schedule activation still depends on service health. But Task 5's core deliverables — correct report generation, Will's format/content sign-off — do not.
...
- [ ] Migration 027 applied. `\d core.documents` shows `privilege`, `confidentiality`, `protective_order_id`. `\d legal.cases` shows `assigned_judge`, `opposing_firm`. `ops.migrations_applied` has `027_legal_privilege_judge` row with `status='applied'`.
- [ ] Migration 028 applied. `\d core.documents` shows `phi_status`, `phi_categories`, `minor_patient`, `phi_authorization_id`. `medical.authorizations` and `medical.disclosures_log` exist. `ops.migrations_applied` has `028_medical_phi_classification` row with `status='applied'`.
...
- [ ] Rollback SQL for 026, 027, and 028 exists, has been tested in shadow schema, and an end-to-end dry rollback has been executed once in staging.
...
2. Apply `028_medical_phi_classification_down.sql` via migration runner (reverses 028 if applied). Runner updates the `ops.migrations_applied` row to `status='rolled_back'`.
3. Apply `027_legal_privilege_judge_down.sql` via migration runner (reverses 027 if applied). Same status update.
...
5. For `ops.pending_approvals` rows with `decision_type` prefix `sprint_a_*` **where `approved IS NULL`** (still pending): set `approved=false`, `rejection_note='sprint_aborted'`, `approval_method='api'` (the rollback is performed by the rollback script, not via conversation). Rows where `approved` is already `true` or `false` are **not modified** — flipping a previously-approved row to `approved=false` would falsify the audit trail. The downstream reversion of work those rows authorized is recorded in step 7's finding, not by mutating the approval row itself.
...
6. **Human quiet hours respected for autonomous work.** No new `ops.pending_approvals` entries created autonomously between 10pm and 7am PT. **"Autonomous" means agent-initiated without current user direction** — i.e., a scheduled job, a background worker, a self-triggered follow-up action. Any action taken inside a live human-directed conversation is by definition *directed*, not autonomous, and proceeds regardless of time of day. The quiet-hours rule gates background/self-scheduled work only. If Will is in conversation with the agent at 11pm and directs work that produces a new `ops.pending_approvals` entry, the rule does not fire — Will is actively present and the gate is synchronous.
...
9. **Conversational-approval mechanics.** When Will approves a stop-gate in live conversation (e.g. "approved" in chat), the agent records the approval to `ops.pending_approvals` as its next action *before* acting on it:
   ...
   UPDATE ops.pending_approvals
    ...
    WHERE decision_type = '<gate>' AND approved IS NULL;
   ```

   On rejection ("no, don't ship that") the agent records `approved=false`, `rejection_note=<Will's stated reason>`, `approval_method='conversation'`. On ambiguous or partial answers ("ship the legal part but not the medical part"), the agent does NOT auto-record — it asks a clarifying question and creates separate `ops.pending_approvals` entries if needed. **The table stays the single source of truth for approval state.** Rule 1's polling convention is unchanged — on any subsequent step, the agent reads from the table, not from conversation memory.

   `approval_method` values: `conversation` (directed chat with Will), `dashboard` (future UI click), `api` (direct row update, e.g. during rollback scripts), `bootstrap` (the single exception for migration 026 itself, which cannot pass through this flow). NULL on historical rows predating this rule.
...
**Design note for the eventual refactor sprint.** The decision of which of the two sets becomes canonical is non-trivial and should be made deliberately, not by accident of "whichever is easier to import from the other."

- `mcp-server/mcp_server_v2.py` is the broader, more general surface. It's consumed by Claude Desktop, Claude Code, and any future MCP client. Its tool contracts are shaped by MCP's protocol expectations (JSON-serializable returns, self-describing arg schemas, tool annotations for read/write/destructive).
- `dashboard/app.py` has UX-coupled logic the MCP layer does not need: pagination for the chat UI, progress indicators, result formatting for in-browser display, user-session state tied to the dashboard's auth model.

The clean refactor is therefore likely **not** "dashboard imports from mcp_server" (which would couple UX concerns into an MCP module or leak MCP concerns into the dashboard). It's more likely "both import from a shared retrieval library, and each adds its own presentation layer on top." The shared library owns the SQL, the safety filter, the access log write. The MCP surface adds MCP-tool wrapping and annotation metadata. The dashboard adds pagination, chat-UI formatting, and user-session-aware logging attribution. This framing also means the Sprint A safety filter ends up in exactly the right place — the shared library — the moment the refactor lands, because Sprint A has already made `core_safety.py` the single implementation point.

Call this out in the closeout so the eventual refactor sprint has a design direction to evaluate against, rather than starting from a blank slate and re-litigating the canonical-set question.
