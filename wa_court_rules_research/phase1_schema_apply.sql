-- =============================================================================
-- WA Court Rules Phase 1 Schema Apply — TRANSACTIONAL
-- =============================================================================
-- Generated by:  WA Court Rules ingest agent (Phase 1 staging)
...
--   - PostgreSQL 17 + pgvector loaded
--   - core.tags, core.documents, rcw.*, usc.*, wac.* schemas exist
--   - Phase 0 close-out artifacts in /opt/wdws/wa_court_rules_research/ — see
--     phase0_cumulative_report.md for reconciled scope and locked decisions
--
-- This script is atomic. Either the entire transaction commits or nothing
-- changes. Any pre-flight or post-apply guard failure RAISES EXCEPTION and
-- the transaction is automatically ROLLed BACK by ON_ERROR_STOP.
--
-- Companion documents:
--   /opt/wdws/wa_court_rules_research/phase1_schema_design.sql  (review-only)
--   /opt/wdws/wa_court_rules_research/phase1_regex_test.py      (T17 unit test)
--   /opt/wdws/wa_court_rules_research/phase0_cumulative_report.md
-- =============================================================================
--
-- HOW TO APPLY (operator instructions):
--
--   psql "$DATABASE_URL" -f /opt/wdws/wa_court_rules_research/phase1_schema_apply.sql
--
-- Expected behavior:
--   1. §0 pre-flight guards print "PASS" NOTICE or RAISE EXCEPTION
--      (verifies 0 'wa-court-rule' short-form rows on BOTH rcw + wac;
--       wa_court_rules schema absent; state-wa-court-rule slug absent;
--       corpora intact: rcw=51,764 wac=73,180 usc=11,678)
--   2. §HARMONIZE drops + recreates chk_target_jurisdiction on BOTH
--      rcw.cross_references AND wac.cross_references in same transaction
--      ('wa-court-rule' short form replaced by 'state-wa-court-rule')
--   3. §1 wa_court_rules schema + 5 tables (rule_sets, rules, cross_references,
--      rule_history, ingestion_runs)
--   4. §2 indexes (incl. partial idx on range_doc_id)
--   5. §3 tag inserts (1 code-jurisdiction slug + 3 ghost-flag tags +
--      15 in-scope rule-set tags + 10 topical tags = 29 new rows)
--   6. §POST guards print "PASS" NOTICE or RAISE EXCEPTION
--      (verifies all three CHECK constraints harmonized; tags seeded;
--       no 'wa-court-rule' short form anywhere)
--   7. §V verification SELECTs print to stdout for operator review
--   8. COMMIT succeeds OR full ROLLBACK on any failure
--
-- Capture entire psql output (stdout + stderr) and surface to agent for
-- parsing and Phase 2 acquisition authorization decision.
--
-- Estimated wall time: under 15 seconds. DDL is metadata-only on rcw + wac
-- (DROP/RECREATE CHECK with 0 affected rows = free) + new schema creation
-- + 29 tag inserts. No large UPDATEs.
-- =============================================================================
--
...
\echo ''
\echo '=== Phase 1 schema apply: COMPLETE ==='
\echo 'Phase 2 acquisition can now begin per kickoff §6 boundary.'
